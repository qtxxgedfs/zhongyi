...
未解字所在的最小完整语义单元标记为 `quality_status=quarantined`、`citation_allowed=false`、`search_allowed=false`，并把 `text_search` 置空。当前288个隔离段只占5,655字符；可引用语料中未解字为零。《温病条辨》两个经影像确认的缺字通过 `sources/normalization/passage-text-corrections.json` 在保持段落 ID 的前提下修订并解除隔离。
...
《温病条辨》两个原核心隔离单元已由影像确认并修订，当前239条段落全部可引用，不再构成来源级阻断。
...
当前共检出35个显式缺字标记、340个 `HT/KT` 旧转录占位符和1处界面文字混入：

- `commentary-neijing-leijing`：2个显式缺字；
- `commentary-jingui-xubin`：2个未确认 `SKchar`；
- `commentary-suwen-wangbing`：31个显式缺字/空字符模板（另有低校对页阻断）；
- 13种医家来源含340个 `HT/KT` 占位符：灵枢张志聪本、成无己本、方有执本、柯琴本、伤寒尤怡本、金匮尤怡本、陈修园本、徐大椿本、《难经集注》、《温热经纬》、《时病论》、素问张志聪本及《灵枢识》；
- `commentary-nanjing-jizhu`：1处“滚动条”界面文字混入。

`HT/KT` 在不同上下文中代表的原字并不相同，不能全局替换，必须逐条查扫描或可靠版本。医家扩展中的未解单元已完整披露并隔离，不再单独阻断；六部核心当前均无隔离段。
...
当前完整测试为48项。测试覆盖26种作品、973页哈希、938页纳入状态、普通及可引用段落链接、700字符上限、可引用语料零未解字、288条隔离披露、模板/HTML/替换字符/PUA 清理、影像确认修订、医学“寫/泻”阅读转换、逐页署名清单及“候选锁不得冒充最终锁”。
